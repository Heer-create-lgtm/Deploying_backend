...
exports.statsAggregator = exports.StatsAggregator = void 0;
const Event_1 = require("../models/Event");
const VariantStats_1 = require("../models/VariantStats");
const ScoringService_1 = require("../services/ScoringService");
const DEFAULT_INTERVAL_MS = parseInt(process.env.STATS_AGGREGATION_INTERVAL_MS || '300000', 10); // 5 minutes
...
class StatsAggregator {
    ...
    async aggregate() {
        ...
        try {
            ...
            for (const [key, data] of variantData) {
                ...
                let stats = await VariantStats_1.VariantStats.findOne({ variant_id, hub_id: data.hub_id });
                if (!stats) {
                    stats = new VariantStats_1.VariantStats({
                        variant_id,
                        hub_id: data.hub_id,
                        clicks: 0,
                        impressions: 0,
                ...
                const recentClicks = await Event_1.Event.countDocuments({
                ...
                stats.score = (0, ScoringService_1.calculateScore)(stats.clicks, stats.impressions, stats.ctr, stats.recent_clicks_hour);
                stats.last_updated = new Date();
                await stats.save();
                // Mark events as processed
                await Event_1.Event.updateMany({ _id: { $in: data.event_ids } }, { $set: { processed: true } });
            }
     ...
     */
    async updateRecentClicksForAllVariants() {
        try {
            const allStats = await VariantStats_1.VariantStats.find({});
            const oneHourAgo = new Date(Date.now() - ONE_HOUR_MS);
    ...
    async getHubStats(hubId) {
        return VariantStats_1.VariantStats.find({ hub_id: hubId })
            .sort({ score: -1 })
            .lean();
    }
    /**
     ...
     */
    async resetStats(variantId) {
        await VariantStats_1.VariantStats.deleteOne({ variant_id: variantId });
        console.log(`Reset stats for variant ${variantId}`);
    }
}
exports.StatsAggregator = StatsAggregator;
// Singleton instance
exports.statsAggregator = new StatsAggregator();
