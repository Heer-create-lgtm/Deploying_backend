 ...
 */
async function seedHub001() {
    console.log('Seeding hub_001...');
    // Connect to MongoDB
    await (0, database_1.connectMongoDB)();
    const HUB_ID = 'hub_001';
    // Clean up existing data for this hub
    await LinkHub_1.LinkHub.deleteMany({ hub_id: HUB_ID });
    await RuleTree_1.RuleTree.deleteMany({ hub_id: HUB_ID });
    await Variant_1.Variant.deleteMany({ hub_id: HUB_ID });
    await VariantStats_1.VariantStats.deleteMany({ hub_id: HUB_ID });
    ...
    const tree = await RuleTree_1.RuleTree.create({
        name: 'ruletree',
        hub_id: HUB_ID,
        root: ruleTree,
        version: 1,
    ...
    await LinkHub_1.LinkHub.updateOne({ hub_id: HUB_ID }, { $set: { rule_tree_id: tree._id } });
    ...
    for (const v of variants) {
        await Variant_1.Variant.create({
            ...v,
            hub_id: HUB_ID,
            enabled: true,
        });
...
//# sourceMappingURL=seedHub001.js.map